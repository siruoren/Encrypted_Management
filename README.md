...
10. 系统凭证管理页面点击「外部存储」弹窗启用并配置外部存储后端，可自定义存储路径和加密密码，同步凭据
11. 系统凭证管理页面点击「审计日志」弹窗查看所有凭据操作记录（按时间倒序），可设置最大保留天数
12. 点击页面右上角「API 文档」按钮进入 Swagger 风格 API 文档页面，可查看所有 API 端点详情并在线测试
13. 系统凭证管理页面点击「全量导出(ZIP)」可导出所有目录任务凭据为加密 ZIP 包，点击「全量导入(ZIP)」可从 ZIP 包恢复凭据
...
- **ZIP 全量导入导出优化**：全量导入 ZIP 分为两步：先解析 ZIP 展示所有凭据（按目录分组，默认全选），再选择要导入的凭据；冲突处理支持"跳过已存在的凭据"（默认）和"强制覆盖已存在的凭据"；导入结果弹窗显示每条凭据的目录任务路径

- **导入速度优化**：新增 `ImportService` 统一导入服务，使用线程池并发导入提升批量导入速度；使用 daemon 线程防止内存泄露
...
- **安全增强**：
  ...
  - Zip Slip 漏洞修复：对 ZIP 条目路径进行规范化检查
  - 密码安全管理：外部存储密码从 `String` 改为 `char[]`，使用后及时擦除
  - 加密密钥与 Jenkins master.key 绑定，防止离线暴力破解
  - JSON Schema 验证：凭据类型白名单、字段白名单和长度校验
  - 审计日志脱敏：credentialId 和 folder 使用 SHA-256 哈希摘要记录
  - 导入限流：ZIP 文件大小限制 20MB，单次导入凭据数量限制 1000 条
  - 线程池生命周期管理：Jenkins 关闭时优雅关闭所有线程池
