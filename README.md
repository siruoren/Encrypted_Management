...
- **全量 ZIP 导入导出**：系统凭证管理页面支持导入导出所有目录任务的凭据并打包为 ZIP 包，ZIP 包结构与外部存储路径一致；导出时使用 AES-256-GCM 加密保护；导入时分为两步：先解析 ZIP 展示凭据列表（按目录分组，默认全选），再选择要导入的凭据；支持"跳过已存在"和"强制覆盖"两种冲突处理策略
