...
- **外部存储功能精简**：移除目录任务（Folder）级别的外部存储功能，仅保留系统凭据管理下的外部存储功能，减少凭据扩散风险

- **审计日志功能精简**：移除目录任务下的审计日志功能，仅保留系统凭据管理下的审计日志，条目按时间倒序排列

- **加密代码去重**：AES-256-GCM 加密/解密/PBKDF2 密钥派生代码统一提取到 `CryptoService`

- **异常信息脱敏**：前端不再返回异常详情，仅显示通用错误提示

- **XSS 防护完善**：所有返回前端的用户可控字段统一 HTML 转义
